...
def decode(input):
    ...
    for line in input.strip().split("\n"):
        ...
        if is_mapping:
            ...
            mapping_name = parts[0][0]
            mapping_str = parts[0][1]
            mapping_items_str = mapping_str.split(",")
            mapping_items = []
            for item_str in mapping_items_str:
                if ":" not in item_str:
                    mapping_items.append(Mapping("", "", 0, item_str))
                else:
                    item_parts = item_str.split(":")
                    comparison_parts = re.findall(r"(\w+)([<>])(\d+)", item_parts[0])
                    mapping_items.append(
                        Mapping(
                            comparison_parts[0][0],
                            comparison_parts[0][1],
                            int(comparison_parts[0][2]),
                            item_parts[1],
                        )
                    )
            mapping[mapping_name] = mapping_items
        else:
            piece = {}
            for attribute_str in re.findall(r"{(.+)}", line)[0].split(","):
                attribute_parts = re.findall(r"(\w+)=(\d+)", attribute_str)
                piece[attribute_parts[0][0]] = int(attribute_parts[0][1])
            pieces.append(piece)
    return Game(mapping, pieces)


def go_through_workflow(piece, mapping):
    output = "in"
    while output not in ["A", "R"]:
        workflows = mapping[output]
        for workflow in workflows:
            if (
                workflow.operator == ""
                or (
                    workflow.operator == "<"
                    and piece[workflow.input] < workflow.compared_target
                )
                or (
                    workflow.operator == ">"
                    and piece[workflow.input] > workflow.compared_target
...
def play(input):
    ...
    for piece in game.pieces:
        if go_through_workflow(piece, game.mapping):
            # print(f"piece{piece}=A")
            result += sum(piece.values())
        # else:
        # print(f"piece{piece}=R")
    return result
...
def intersect(a, b):
    ...
    non_intersected_left = (a[0], intersection[0] - 1)
    non_intersected_right = (intersection[1] + 1, a[1])
    non_intersection_list = []
    if non_intersected_left[0] <= non_intersected_left[1]:
        non_intersection_list.append(non_intersected_left)
    if non_intersected_right[0] <= non_intersected_right[1]:
        non_intersection_list.append(non_intersected_right)
    return intersection, non_intersection_list


def go_through_workflow_bfs(input_intervals, workflows_list):
    task_queue = [(input_intervals, "in", 0)]
    accepted = []
    while len(task_queue) > 0:
        intervals, workflow_name, workflow_index = task_queue.pop(0)
        if workflow_name in ["A", "R"]:
            if workflow_name == "A":
                accepted.append(intervals)
            continue
        workflows = workflows_list[workflow_name]
        workflow = workflows[workflow_index]
        if workflow_index == len(workflows) - 1:
            task_queue.append((intervals, workflow.output, 0))
        else:
            a = intervals[workflow.input]
            b = (1, workflow.compared_target - 1)
            if workflow.operator == ">":
                b = (workflow.compared_target + 1, 4000)
            intersection, non_intersection_list = intersect(a, b)
            if intersection:
                new_intervals = intervals.copy()
                new_intervals[workflow.input] = intersection
                task_queue.append((new_intervals, workflow.output, 0))
            for non_intersection in non_intersection_list:
                new_intervals = intervals.copy()
                new_intervals[workflow.input] = non_intersection
                task_queue.append((new_intervals, workflow_name, workflow_index + 1))
    return accepted


def calculate_combination(input):
    result = 0
    for intervals in input:
        partial_result = 1
        for interval in intervals.values():
            partial_result *= interval[1] - interval[0] + 1
...
def play_2(input):
    game = decode(input)
    input_interval = {"x": (1, 4000), "m": (1, 4000), "a": (1, 4000), "s": (1, 4000)}
    accepted = go_through_workflow_bfs(input_interval, game.mapping)
    return calculate_combination(accepted)
