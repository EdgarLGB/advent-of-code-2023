...
def decode(input):
    ...
    for i, line in enumerate(input.split('\n')):
        ...
        for j, location in enumerate(line):
            match location:
                case '.':
                    maze_line.append(Piece({}, location))
                case '|':
                    maze_line.append(
                        Piece({north: north, south: south}, location))
                case '-':
                    maze_line.append(Piece({east: east, west: west}, location))
                case 'L':
                    maze_line.append(
                        Piece({south: east, west: north}, location))
                case 'J':
                    maze_line.append(
                        Piece({south: west, east: north}, location))
                case '7':
                    maze_line.append(
                        Piece({east: south, north: west}, location))
                case 'F':
                    maze_line.append(
                        Piece({west: south, north: east}, location))
                case 'S':
                    maze_line.append(Piece({}, location))
                    starting_point = [i - 1, j]
        maze.append(maze_line)
    return Game(maze, starting_point)


def calculate_number_of_steps(input):
    game = decode(input)
    probes = list(initiateProbes(game.starting_point))
    return calculate_length_of_loop_in_two_probes(probes, game.maze)


def initiateProbes(starting_point):
    for d in [north, south, west, east]:
        yield Probe(starting_point[0], starting_point[1], d)


def calculate_length_of_loop_in_two_probes(probes, maze):
    result = 0
    while True:
        next_probes = []
        for probe in probes:
            next_probe_i = probe.i + probe.direction.i
            next_probe_j = probe.j + probe.direction.j
            if next_probe_i >= 0 and next_probe_i < len(maze) and next_probe_j >= 0 and next_probe_j < len(maze[probe.i]):
                next_piece = maze[next_probe_i][next_probe_j]
                next_possible_direction = next_piece.connection.get(
                    probe.direction)
                if next_possible_direction:
                    next_probes.append(
                        Probe(next_probe_i, next_probe_j, next_possible_direction))
        result += 1
        probes = next_probes
        if probes[0].i == probes[1].i and probes[0].j == probes[1].j:
            return result


# print(calculate_number_of_steps(input))
# print(calculate_number_of_steps(input2))
# print(calculate_number_of_steps(input3))

###### Part two ######
def calculate_number_of_enclosed_tiles(input):
    game = decode(input)
    starting_directions = list(find_starting_directions(
        game.starting_point, game.maze))
    main_loop = remove_useless_pipe(game.starting_point, starting_directions[0], game.maze)
    result = []
    for starting_direction in starting_directions:
        result.append(paint_enclosed_tiles(
            game.starting_point, starting_direction, game.maze))
    print_maze(game.maze, main_loop, result[0])
    return result
...
colorama_init()
def print_maze(maze, main_loop, enclosed_tiles):
    for i in range(len(maze)):
        line = ''
        for j in range(len(maze[0])):
            if Coordinate(i, j) in main_loop:
                line += f'{Fore.RED}{maze[i][j].symbol}{Style.RESET_ALL}'
            elif Coordinate(i, j) in enclosed_tiles:
                line += f'{Fore.BLUE}X{Style.RESET_ALL}'
            else:
                line += maze[i][j].symbol
        print(line)

def find_starting_directions(starting_point, maze):
    for direction in [north, south, east, west]:
        next_point_i = starting_point[0] + direction.i
        next_point_j = starting_point[1] + direction.j
        if next_point_i >= 0 and next_point_i < len(maze) and next_point_j >= 0 and next_point_j < len(maze[0]):
            next_piece = maze[next_point_i][next_point_j]
            if direction in next_piece.connection:
                yield direction
...
def remove_useless_pipe(starting_point, starting_direction, maze):
    current_point = [starting_point[0] + starting_direction.i,
                     starting_point[1] + starting_direction.j]
    entry_direction = starting_direction
    main_loop = {Coordinate(starting_point[0], starting_point[1])}
    exit_direction = None
    while True:
        main_loop.add(Coordinate(current_point[0], current_point[1]))
        exit_direction = maze[current_point[0]
                              ][current_point[1]].connection[entry_direction]
        current_point[0] += exit_direction.i
        current_point[1] += exit_direction.j
    ...
    for i in range(len(maze)):
        for j in range(len(maze[0])):
            if Coordinate(i, j) not in main_loop:
                maze[i][j] = Piece({}, '.')
    return main_loop
    # print(main_loop)
    # print(maze)
    # maze[starting_point[0]][starting_point[1]] = Piece(
        # {starting_direction: exit_direction, exit_direction: starting_direction}, 'S')


def paint_enclosed_tiles(starting_point, starting_direction, maze):
    current_point = [starting_point[0] + starting_direction.i,
                     starting_point[1] + starting_direction.j]
    entry_direction = starting_direction
    enclosed_tiles = set()
    while True:
        ...
        for direction in [entry_direction, exit_direction]:
            if north == direction:
                paint_dfs(current_point, east, maze, enclosed_tiles)
            elif east == direction:
                paint_dfs(current_point, south, maze, enclosed_tiles)
            elif south == direction:
                paint_dfs(current_point, west, maze, enclosed_tiles)
            elif west == direction:
                paint_dfs(current_point, north, maze, enclosed_tiles)
...
def paint_dfs(current_point, painting_direction, maze, enclosed_tiles):
    painting_point = [current_point[0] + painting_direction.i,
                      current_point[1] + painting_direction.j]
    hashable_painting_point = Coordinate(painting_point[0], painting_point[1])
    if painting_point[0] >= 0 and painting_point[0] < len(maze) and painting_point[1] >= 0 and painting_point[1] < len(maze[0]) \
            and maze[painting_point[0]][painting_point[1]].symbol == '.':
        enclosed_tiles.add(hashable_painting_point)
        paint_dfs(painting_point, painting_direction, maze, enclosed_tiles)


result = calculate_number_of_enclosed_tiles(input3)
# print(result)
