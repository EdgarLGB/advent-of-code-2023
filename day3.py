...
def calculate_sum_of_part_number(input):
    lines = decode(input)
    result = 0
    for i in range(len(lines)):
        ...
        for j in range(len(lines[i])):
            ...
            is_current_char_digit = is_digit(current_char)
            if is_current_char_digit is True:
                part_number = part_number * 10 + int(current_char)
                is_valid |= has_symbol(lines, i, j)
            if is_current_char_digit is False or j == len(lines[i]) - 1:
                if is_valid is True:
                    result += part_number
                part_number = 0
                is_valid = False    # Reset the parameters
    return result
...
def is_symbol(input, i, j):
    if i >= 0 and i < len(input) and j >= 0 and j < len(input[i]):
        result = is_digit(input[i][j]) is False and input[i][j] != '.'
        return result
    return False


Orientation = namedtuple('Orientation', ['di', 'dj'])
orientations = [Orientation(0, 1), Orientation(-1, 1), Orientation(-1, 0), Orientation(-1, -1),
                Orientation(0, -1), Orientation(1, -1), Orientation(1, 0), Orientation(1, 1)]

def has_symbol(input, i, j):
    result = False
    for orientation in orientations:
        result |= is_symbol(input, i + orientation.di, j + orientation.dj)
    return result


print(calculate_sum_of_part_number(input))

###### Part two ######


def calculate_sum_of_gear_ratio(input):
    lines = decode(input)
    part_numbers_around_gears = calculate_part_numbers_around_gears(lines)
    result = 0
    for part_numbers in part_numbers_around_gears.values():
...
def calculate_part_numbers_around_gears(lines):
    result = {}  # gear location -> a list of part number
    for i in range(len(lines)):
        possible_part_number = 0
        gear_locations = set()  # A hashset of gear locations
        for j in range(len(lines[i])):
            current_char = lines[i][j]
            if is_digit(current_char) is True:
                possible_part_number = possible_part_number * 10 + int(current_char)
                gear_locations.update(locate_gears(lines, i, j))
            if is_digit(current_char) is False or j == len(lines[i]) - 1:
                carry_part_number(gear_locations, possible_part_number, result)
                possible_part_number = 0
                gear_locations = set()
    return result


def is_gear(input, location):
    i, j = location.i, location.j
    if i >= 0 and i < len(input) and j >= 0 and j < len(input[i]):
        return input[i][j] == '*'
    return False
...
def locate_gears(lines, i, j):
    result = []
    for orientation in orientations:
        location = Location(i + orientation.di, j + orientation.dj)
        if is_gear(lines, location) is True:
            result.append(location)
    return result


def carry_part_number(gear_locations, part_number, result):
    for gear_location in gear_locations:
        if gear_location in result:
            result[gear_location].append(part_number)
        else:
            result[gear_location] = [part_number]
...
print(calculate_sum_of_gear_ratio(input))
