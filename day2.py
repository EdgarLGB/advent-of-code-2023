...
Set = namedtuple('Set', ['red', 'blue', 'green'])


def extract_color(color: str, input: str):
    matched_groups = re.search(f'\D*(\d+)\D*{color}', input)
    if matched_groups:
        return int(matched_groups.group(1))
    return 0
...
def decode(input: str):
    for line in input.split('\n'):
        if line == "":
            continue
        game_and_set = line.split(':')
        game_id = int(re.match('Game (\d+)', game_and_set[0]).group(1))
        sets = game_and_set[1].split(';')
        result_sets = []
        for s in sets:
            red = extract_color('red', s)
            blue = extract_color('blue', s)
            green = extract_color('green', s)
            result_sets.append(Set(red, blue, green))
        yield Game(game_id, result_sets)


def calculate_possible_games(input, red_cubes, blue_cubes, green_cubes):
    games = decode(input)
    result = 0
    for game in games:
        is_valid = True
        for s in game.sets:
            if s.red > red_cubes or s.blue > blue_cubes or s.green > green_cubes:
                is_valid = False
                break
        if is_valid is True:
            result += game.id
...
print(calculate_possible_games(input, 12, 14, 13))
...
def calculate_power(input):
    ...
    for game in games:
        ...
        min_blue = 0
        for s in game.sets:
            min_red = max(min_red, s.red)
            min_green = max(min_green, s.green)
            min_blue = max(min_blue, s.blue)
