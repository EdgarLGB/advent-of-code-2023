...
def calibrate(input):
    result = 0
    for line in input:
        left_digit = locate_number(line, True)
        right_digit = locate_number(line, False)
        if left_digit is None or right_digit is None:
            continue
...
def locate_number(input: str, is_forward: bool):
    if len(input) == 0:
        return
    pointer = 0 if is_forward else len(input) - 1
    while pointer >= 0 and pointer < len(input):
        current_char = input[pointer]
        if current_char >= '0' and current_char <= '9':
            return ord(current_char) - ord('0')
        if is_forward:
            pointer += 1
        else:
            pointer -= 1
    ...
    for line in input.split('\n'):
        ...
        while pointer < len(line):
            ...
            for k, v in digit_dic.items():
                if line.startswith(k, pointer):
                    result += v
                    pointer += len(k)
                    has_match = True
                    break
                if line.startswith(k, pointer - 1): # The last trailing character is borrowed to build a word, for example, eighthree, the t character is borrowed from eight to create a three
                    result += v
                    pointer += len(k) - 1
                    has_match = True
                    break
...
lines = digitalize(input)
print(calibrate(lines))
